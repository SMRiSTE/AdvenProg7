...
public:
    ...
    std::unique_ptr<tridiagonal_matrix> clone() {
        std::vector<double>& clone_down = m_down;
        const std::vector<double>& clone_upper = m_upper;
        const std::vector<double>& clone_middle = m_middle;
        return std::make_unique<tridiagonal_matrix>(clone_down, clone_middle, clone_upper);
    }
};
